...
const DetailsThread = () => {
    ...
    const axiosData = (page) => {
        Promise.all([
            axios.get(`http://localhost:9999/api/threads/${params.id}`),
            axios.get(`http://localhost:9999/api/threads/${params.id}/posts?page=${page}&limit=10`),
        ])
            .then(([thread, posts]) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (isEdit) {
            let resEdit = await axios.put(`http://localhost:9999/api/posts/${idPost}`, { content: editorHtml },
            { headers: { "x-access-token": localStorage.getItem('x-access-token') } })
        }
        if (isNewPost) {
            let res = await axios.post(`http://localhost:9999/api/posts/`, {
                content: editorHtml,
    ...
    const deletePost = async (id) => {
        let res = await axios.delete(`http://localhost:9999/api/posts/${id}`, { headers: { "x-access-token": localStorage.getItem('x-access-token') } });
        axiosData();
    }
